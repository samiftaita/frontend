import React from "react";
import { NavLink, Link } from "react-router-dom";
import "../../styles/admin.css";

/* ── Icônes SVG ── */
const Icons = {
  dashboard: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <rect x="3" y="3" width="7" height="7" rx="1" />
      <rect x="14" y="3" width="7" height="7" rx="1" />
      <rect x="3" y="14" width="7" height="7" rx="1" />
      <rect x="14" y="14" width="7" height="7" rx="1" />
    </svg>
  ),
  profile: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" />
      <circle cx="12" cy="7" r="4" />
    </svg>
  ),
  tooth: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M8 3C6 3 4 5 4 7.5C4 9.5 5 11 5.5 13C6 15 6 17 6 19C6 20.1 6.4 21 7.5 21C8.5 21 9 20 9.5 18.5C10 17 10.5 16 12 16C13.5 16 14 17 14.5 18.5C15 20 15.5 21 16.5 21C17.6 21 18 20.1 18 19C18 17 18 15 18.5 13C19 11 20 9.5 20 7.5C20 5 18 3 16 3C14.8 3 13.5 3.8 12 3.8C10.5 3.8 9.2 3 8 3Z" />
      <path
        d="M10 6C10.5 7 11.2 7.5 12 7.5C12.8 7.5 13.5 7 14 6"
        strokeWidth="1.4"
        opacity="0.6"
      />
    </svg>
  ),
  services: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M22 12h-4l-3 9L9 3l-3 9H2" />
    </svg>
  ),
  calendar: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <rect x="3" y="4" width="18" height="18" rx="2" />
      <line x1="16" y1="2" x2="16" y2="6" />
      <line x1="8" y1="2" x2="8" y2="6" />
      <line x1="3" y1="10" x2="21" y2="10" />
    </svg>
  ),
  rdv: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <rect x="3" y="4" width="18" height="18" rx="2" />
      <line x1="16" y1="2" x2="16" y2="6" />
      <line x1="8" y1="2" x2="8" y2="6" />
      <line x1="3" y1="10" x2="21" y2="10" />
      <polyline points="9 16 11 18 15 14" />
    </svg>
  ),
  folder: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z" />
    </svg>
  ),
  clipboard: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2" />
      <rect x="8" y="2" width="8" height="4" rx="1" />
      <line x1="9" y1="12" x2="15" y2="12" />
      <line x1="9" y1="16" x2="13" y2="16" />
    </svg>
  ),
  logout: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      className="w-5 h-5"
    >
      <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" />
      <polyline points="16 17 21 12 16 7" />
      <line x1="21" y1="12" x2="9" y2="12" />
    </svg>
  ),
  chevronLeft: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      width="16"
      height="16"
    >
      <polyline points="15 18 9 12 15 6" />
    </svg>
  ),
  chevronRight: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      viewBox="0 0 24 24"
      width="16"
      height="16"
    >
      <polyline points="9 18 15 12 9 6" />
    </svg>
  ),
  close: (
    <svg
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      viewBox="0 0 24 24"
      className="w-4 h-4"
    >
      <line x1="18" y1="6" x2="6" y2="18" />
      <line x1="6" y1="6" x2="18" y2="18" />
    </svg>
  ),
};

const LINKS = [
  { to: "/admin/dashboard", label: "Tableau de bord", icon: Icons.dashboard },
  { to: "/admin/profile", label: "Profil", icon: Icons.profile },
  { to: "/admin/dentistes", label: "Dentistes", icon: Icons.tooth },
  { to: "/admin/services", label: "Services", icon: Icons.services },
  {
    to: "/admin/disponibilites",
    label: "Disponibilités",
    icon: Icons.calendar,
  },
  { to: "/admin/rendez-vous", label: "Rendez-vous", icon: Icons.rdv },
  {
    to: "/admin/dossiers-medicaux",
    label: "Dossiers médicaux",
    icon: Icons.folder,
  },
  {
    to: "/admin/fiches-soins",
    label: "Fiches de soins",
    icon: Icons.clipboard,
  },
];

/* Logo dent SVG */
const ToothLogo = () => (
  <svg
    viewBox="0 0 100 100"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    width="22"
    height="22"
  >
    <path
      d="M32 8C24 8 16 15 16 25C16 33 20 40 22 48C24 56 24 64 24 73C24 78 26 82 30 82C34 82 36 78 38 73C40 68 42 64 50 64C58 64 60 68 62 73C64 78 66 82 70 82C74 82 76 78 76 73C76 64 76 56 78 48C80 40 84 33 84 25C84 15 76 8 68 8C63 8 57 11 50 11C43 11 37 8 32 8Z"
      fill="white"
    />
    <path
      d="M36 20C40 26 44 28 50 28C56 28 60 26 64 20"
      stroke="#bae6fd"
      strokeWidth="5"
      fill="none"
      strokeLinecap="round"
    />
  </svg>
);

const AdminSidebar = ({
  open = false,
  onClose = () => {},
  onLogout = () => {},
  collapsed = false,
  onToggleCollapse = () => {},
}) => {
  return (
    <aside
      className={`admin-sidebar ${open ? "open" : ""} ${collapsed ? "collapsed" : ""}`}
    >
      {/* ── Header ── */}
      <div className="admin-sidebar-header">
        {collapsed ? (
          /* Mode réduit : bouton expand centré */
          <button
            type="button"
            className="sidebar-collapse-btn sidebar-collapse-btn-center"
            onClick={onToggleCollapse}
            aria-label="Agrandir le menu"
            title="Agrandir le menu"
          >
            {Icons.chevronRight}
          </button>
        ) : (
          /* Mode normal : logo + bouton collapse */
          <>
            <Link
              to="/"
              style={{ textDecoration: "none" }}
              className="admin-brand"
            >
              <div className="admin-brand-icon">
                <ToothLogo />
              </div>
              <div>
                <h2 className="admin-brand-name">
                  <span>DENT</span>ORA
                </h2>
                <p className="admin-brand-subtitle">Espace administrateur</p>
              </div>
            </Link>
            <div className="sidebar-header-actions">
              <button
                type="button"
                className="sidebar-collapse-btn"
                onClick={onToggleCollapse}
                aria-label="Réduire le menu"
                title="Réduire le menu"
              >
                {Icons.chevronLeft}
              </button>
              <button
                type="button"
                className="admin-sidebar-close"
                onClick={onClose}
                aria-label="Fermer"
              >
                {Icons.close}
              </button>
            </div>
          </>
        )}
      </div>

      {/* ── Navigation ── */}
      <nav className="admin-sidebar-nav">
        <div className="admin-nav-section">Menu principal</div>
        {LINKS.map((link) => (
          <NavLink
            key={link.to}
            to={link.to}
            className={({ isActive }) =>
              `admin-sidebar-link ${isActive ? "active" : ""}`
            }
            onClick={onClose}
            title={collapsed ? link.label : undefined}
          >
            <span className="nav-icon">{link.icon}</span>
            <span className="nav-label">{link.label}</span>
          </NavLink>
        ))}
      </nav>

      {/* ── Footer ── */}
      <div className="admin-sidebar-footer">
        <button
          type="button"
          className="admin-logout-btn"
          onClick={onLogout}
          title={collapsed ? "Déconnexion" : undefined}
        >
          <span>{Icons.logout}</span>
          <span className="nav-label">Déconnexion</span>
        </button>
      </div>
    </aside>
  );
};

export default AdminSidebar;
